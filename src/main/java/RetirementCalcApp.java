...
import base.GetYear;
import base.Output;
import base.StringToInt;
import base.UsrInput;

public class RetirementCalcApp {

    public static void main(String[] args) {
        String currentAgeStr = UsrInput.input1();
        String retirementAgeStr = UsrInput.input2();
        int currentAge = StringToInt.convertStr(currentAgeStr);
        int retirementAge = StringToInt.convertStr(retirementAgeStr);
        int currentYear = GetYear.currentYear();
        int yearsLeft = GetYear.calculateYearsLeft(currentAge, retirementAge);
        int retYear = GetYear.retirementYear(currentYear, yearsLeft);
        Output.printOutput(yearsLeft, currentYear, retYear );
    }
}
